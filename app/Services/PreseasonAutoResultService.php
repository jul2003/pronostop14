...
class PreseasonAutoResultService
{
    ...
    public function suggestionsForSeason(Season $season): Collection
    {
        $questions = $season->preseasonQuestions()
            ...
            ->where('answer_type', 'top14_club')
            ->whereNotNull('auto_result_rule')
            ->whereNotNull('auto_result_journee_number')
            ->orderBy('position')
            ->orderBy('id')
            ->get();
    ...
    public function suggestionForQuestion(Season $season, SeasonPreseasonQuestion $question): ?array
    {
        ...
        if (blank($question->auto_result_rule) || blank($question->auto_result_journee_number)) {
            return null;
        }

        $targetJourneeNumber = (int) $question->auto_result_journee_number;

        if ($targetJourneeNumber < 1 || $targetJourneeNumber > 26) {
            return null;
        }

        $resolution = match ($question->auto_result_rule) {
            SeasonPreseasonQuestion::AUTO_RESULT_RULE_TOP14_LEADER => $this->detectGuaranteedLeader($season, $targetJourneeNumber),
            SeasonPreseasonQuestion::AUTO_RESULT_RULE_TOP14_LAST => $this->detectGuaranteedLast($season, $targetJourneeNumber),
            default => null,
        };
        ...
        foreach ($standings as $row) {
            ...
            if ((int) $row['points'] > (int) $maxOtherPoints) {
                return [
                    ...
                    'explanation' => $row['club']->name.' a '.$row['points'].' point(s), et aucun autre club ne peut dépasser '.$maxOtherPoints.' point(s) d’ici la J'.$targetJourneeNumber.'.',
                ];
            }
        }
        ...
        foreach ($standings as $row) {
            ...
            if ((int) $row['max_points'] < (int) $minOtherPoints) {
                return [
                    ...
                    'explanation' => $row['club']->name.' peut atteindre au maximum '.$row['max_points'].' point(s), alors que tous les autres clubs ont déjà au moins '.$minOtherPoints.' point(s) d’ici la J'.$targetJourneeNumber.'.',
                ];
            }
        }
